...
from backbone.vgg import vgg_right224,vgg_left224
...
class SFNet(nn.Module):
    def __init__(self, pretrained=True, num_class=1,bias = False):
        ...
        self.vggrgb = vgg_left224()
        self.vggdep = vgg_right224()
        ...
        self.conv_2048_1024 = nn.Sequential(nn.Conv2d(512, 512, kernel_size=3, stride=1, padding=1, bias=bias),
                                         nn.BatchNorm2d(512),
                                         nn.ReLU(inplace=True)
                                         )
        self.conv_1024_512 = nn.Sequential(nn.Conv2d(512, 256, kernel_size=3, stride=1, padding=1, bias=bias),
                                            nn.BatchNorm2d(256),
                                            nn.ReLU(inplace=True)
                                            )
        self.conv_512_256 = nn.Sequential(nn.Conv2d(256, 128, kernel_size=3, stride=1, padding=1, bias=bias),
                                           nn.BatchNorm2d(128),
                                           nn.ReLU(inplace=True)
                                           )
        self.conv_256_64 = nn.Sequential(nn.Conv2d(128, 64, kernel_size=3, stride=1, padding=1, bias=bias),
                                          nn.BatchNorm2d(64),
                                          nn.ReLU(inplace=True)
                                          )
        ...
        self.conv_512_64 = nn.Sequential(nn.Conv2d(256, 64, kernel_size=3, stride=1, padding=1, bias=bias),
                                         nn.BatchNorm2d(64),
                                         nn.ReLU(inplace=True)
                                         )
        self.conv_1024_64 = nn.Sequential(nn.Conv2d(512, 64, kernel_size=3, stride=1, padding=1, bias=bias),
                                         nn.BatchNorm2d(64),
                                         nn.ReLU(inplace=True)
                                         )
        self.conv_2048_64 = nn.Sequential(nn.Conv2d(512, 64, kernel_size=3, stride=1, padding=1, bias=bias),
                                         nn.BatchNorm2d(64),
                                         nn.ReLU(inplace=True)
                                         )
        ...
        self.conv_256_1_output = nn.Conv2d(128, 1, 1, 1)
        self.conv_512_1_output = nn.Conv2d(256, 1, 1, 1)
        self.conv_1024_1_output = nn.Conv2d(512, 1, 1, 1)
        self.conv_2048_1_output = nn.Conv2d(512, 1, 1, 1)
    def forward(self, left, depth):
        lf1, lf2, lf3, lf4, lf5 = self.vggrgb(left)
        A1_d = self.vggdep.features[:6](depth)


        x_depth = self.dem1(A1_d)
        A2_d = self.vggdep.features[6:13](x_depth)

        rf1 = self.dem2(A2_d)
        A3_d = self.vggdep.features[13:23](rf1)
        ...
        rf2 = self.dem3(A3_d)
        A4_d = self.vggdep.features[23:33](rf2)
        ...
        # print("lf3", lf3.shape)
        rf3 = self.dem4(A4_d)
        A5_d = self.vggdep.features[33:43](rf3)
        ...
        MFB_4_r = self.MFB4(lf4, rf3, MFB_5_r_2)
        ...
        MFB_1_r = self.MFB1(lf1,x_depth,MFB_2_r_2)
