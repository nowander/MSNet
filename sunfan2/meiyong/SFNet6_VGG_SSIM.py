...
from backbone.vgg import vgg_left224,vgg_right224
...
class Kendalltau_Gate(nn.Module):
    ...
    def forward(self, *x):
        ...
        corr = pytorch_ssim.ssim(z, y)
        # corr = calc_corr(z,y)
        # print('p', p)
        z_new1 = torch.multiply(z , corr)
...
class MFB(nn.Module):
    def __init__(self,inchannel):
        ...
        self.upsample = nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True)
        # self.PAM=PAM(inchannel*3)
        # self.CAM = CAM(inchannel*2)
        ...
        self.conv4 = nn.Sequential(
            nn.Conv2d(inchannel * 3, inchannel, kernel_size=(1,7), padding=(0,3)),
            nn.Conv2d(inchannel , inchannel, kernel_size=(7, 1), padding=(3, 0)),
            nn.BatchNorm2d(inchannel),
            nn.ReLU(inplace=True)
        )
        self.conv5 = nn.Sequential(
            nn.Conv2d(inchannel * 2, inchannel, kernel_size=(1, 7), padding=(0, 3)),
            nn.Conv2d(inchannel , inchannel, kernel_size=(7, 1), padding=(3, 0)),
    ...
    def forward(self, *x):
        ...
        PAM = torch.cat((D1,D2,D3),dim=1)
        CAM = torch.cat((D4,D5),dim=1)
        PAM1 = self.row_attention1(self.conv4(PAM))
        # CAM1 = self.conv5(CAM)
        CAM1 = self.row_attention2(self.conv5(CAM))
        D5_1 = PAM1 + CAM1
        # PAM1 = self.PAM(PAM)
        # # print(PAM1.shape)
        # CAM1 = self.CAM(CAM)
        # print(CAM1.shape)
        # D5_1 = self.conv3(PAM1) + self.conv2(CAM1)
        # D5_1 = self.conv_3(D6)
        ...
        # print("D2", D2.shape)
        # print('X3', x3.shape)
        # v_mean = torch.mean(D2,dim=1,keepdim=True)
        # v_max,e = torch.max(D2,dim=1,keepdim=True)
        # # print('v_mean3', v_mean.shape)
        # # print('v_max3', v_max.shape)
        # C1 = torch.cat((v_mean,v_max),dim=1)
        # # print("C!!1",C1.shape)
        # C2 = self.conv_7(C1)
        # # print("C@@@2",C2.shape)
        # C3 = torch.sigmoid(C2)
        # M_out = torch.multiply(D2,C3)
        # # print("M_OUTWW",M_out.shape)
        return M_out
        # else:
        #     x1 = x[0]
        #     x2 = x[1]
...
class SFNet(nn.Module):
    def __init__(self, pretrained=True, num_class=1,bias = False):
        ...
        # self.conv_first1 = conv1x1(3, 3)
        # self.conv_first2 = conv1x1(3, 3)
        # self.left0 = nn.Sequential(vgg.features[:7])
        # # self.left0 = nn.Sequential(vgg.features[7:13], conv3x3(64, 128, 2))
        # self.left1 = nn.Sequential(vgg.features[7:14])
        # self.left2 = nn.Sequential(vgg.features[14:24])
        # self.left3 = nn.Sequential(vgg.features[24:34])
        # self.left4 = nn.Sequential(vgg.features[34:44])
        #
        # self.right0 = nn.Sequential(vgg.features[:7])
        # # self.right0_1 = nn.Sequential(vgg.features[7:13], conv3x3(64, 128, 2))
        # self.right1 = nn.Sequential(vgg.features[7:14])
        # self.right2 = nn.Sequential(vgg.features[14:24])
        # self.right3 = nn.Sequential(vgg.features[24:34])
        # self.right4 = nn.Sequential(vgg.features[34:44])
        ...
        # self.MFB1 = MFB(64,64)
        # self.MFB2 = MFB(256,64)
        # self.MFB3 = MFB(512,256)
        # self.MFB4 = MFB(1024,512)
        # self.MFB5 = MFB(2048,1024)
        self.MFB1 = MFB(64)
        self.MFB2 = MFB(128)
        self.MFB3 = MFB(256)
        self.MFB4 = MFB(512)
        ...
        self.BFF4 = BFF()
        self.BFF5 = BFF()
        self.KGM1 = Kendalltau_Gate(0)
        self.KGM2 = Kendalltau_Gate(1)
        self.KGM3 = Kendalltau_Gate(1)
        self.KGM4 = Kendalltau_Gate(1)
        # self.conv_64_1 = nn.Sequential(nn.Conv2d(64, 1, kernel_size=3, stride=1, padding=1, bias=bias),
        #                                  nn.BatchNorm2d(1),
        #                                  nn.ReLU(inplace=True)
        #                                  )
        # self.conv_M128_128 = nn.Sequential(nn.Conv2d(128, 64, kernel_size=3, stride=1, padding=1, bias=bias),
        #                                  nn.BatchNorm2d(64),
        #                                  nn.ReLU(inplace=True)
        #                                  )
        # self.conv_M256_128 = nn.Sequential(nn.Conv2d(256, 128, kernel_size=3, stride=1, padding=1, bias=bias),
        #                                  nn.BatchNorm2d(128),
        #                                  nn.ReLU(inplace=True)
        #                                  )
        self.conv_512_512 = nn.Sequential(nn.Conv2d(512, 512, kernel_size=3, stride=1, padding=1, bias=bias),
                                           nn.BatchNorm2d(512),
                                           nn.ReLU(inplace=True)
                                           )
        ...
        self.conv_256_128 = nn.Sequential(nn.Conv2d(256, 128, kernel_size=3, stride=1, padding=1, bias=bias),
                                         nn.BatchNorm2d(128),
                                         nn.ReLU(inplace=True)
                                         )
        self.conv_128_64 = nn.Sequential(nn.Conv2d(128, 64, kernel_size=3, stride=1, padding=1, bias=bias),
                                          nn.BatchNorm2d(64),
                                          nn.ReLU(inplace=True)
                                          )
        ...
        self.conv_128_256 = nn.Sequential(nn.Conv2d(128, 256, kernel_size=3, stride=1, padding=1, bias=bias),
                                         nn.BatchNorm2d(256),
                                         nn.ReLU(inplace=True)
                                         )
        self.conv_256_512 = nn.Sequential(nn.Conv2d(256, 512, kernel_size=3, stride=1, padding=1, bias=bias),
                                          nn.BatchNorm2d(512),
                                          nn.ReLU(inplace=True)
                                          )
        ...
        self.conv_128_1_output = nn.Conv2d(128, 1, 1, 1)
    def forward(self, left, depth):
        x_l,lf1,lf2,lf3,lf4 = self.vgg (left)
        x_depth = self.vgg_depth.features[:6](depth)
        x= x_l
        # x = self.resnet.conv1(left)
        # x = self.resnet.bn1(x)
        # x = self.resnet.relu(x)
        # x_l =  self.vgg.maxpool(x)
        ...
        # print("lf1",lf1.shape)
        # print('x_depth',x_depth.shape)
        rf = self.vgg_depth.features[6:13](x_depth)
        # print('rf',rf.shape)
        rf1 = self.dem2(rf)
        # print("rf1", rf1.shape)
        # lf2 = self.resnet.layer2(lf1)  # 256 x 56 x 56
        # print("lf2",lf2.shape)
        rf2 = self.vgg_depth.features[13:23](rf1)
        # print('rf2',rf2.shape)
        rf2 = self.dem3(rf2)
        # print("lf2",lf2.shape)
        # print("rf2",rf2.shape)
        # lf3 = self.resnet.layer3(lf2)  # 256 x 64 x 64
        rf3 = self.vgg_depth.features[23:33](rf2)
        # print("lf3", lf3.shape)
        rf3 = self.dem4(rf3)
        ...
        # print("lf4",lf4.shape)
        rf4 =  self.vgg_depth.features[33:43](rf3)
        # F1 = rf4
        # print("rf41",rf4.shape)
        rf4 = self.dem5(rf4)
        # print("rf42", rf4.shape)

        MFB_5_r = self.MFB5(lf4,rf4)
        #csm_5_r = self.upsample2(csm_5_r)
        # print("MFB_5_r",MFB_5_r.shape)
        MFB_5_r_2 = self.conv_512_512(MFB_5_r)
        ...
        # print("csm_4_r_2",MFB_4_r.shape)
        MFB_4_r_2 = self.conv_512_256(MFB_4_r)
        # MFB_4_r_2 = self.upsample2(MFB_4_r_2)

        # print(lf2.shape)
        # print(rf2.shape)
        # print("csm_4_r_2",MFB_4_r_2.shape)

        MFB_3_r = self.MFB3(lf2, rf2, MFB_4_r_2)
        MFB_3_r_2 = self.conv_256_128(MFB_3_r)
        #MFB_3_r_2 = self.upsample2(MFB_3_r_2)
        # print("csm_3_r_2",MFB_3_r_2.shape)
        # print("lf2",lf2.shape)
        # print("rf2", rf2.shape)
        MFB_2_r = self.MFB2(lf1, rf1, MFB_3_r_2)
        # print("MFB_2_r",MFB_2_r.shape)
        MFB_2_r_2 = self.conv_128_64(MFB_2_r)
       ...
       # M_1 = self.conv_M64_128(M_1)
        ...
        # print("M2",M_2.shape)
        # print("M_2",M_2.shape)
        M_2 = self.conv_128_64(M_2)
        # print("M_2_after", M_2.shape)
        M_2 = self.upsample2(M_2)
        # print("csm_3_r",MFB_3_r.shape)
        M_3 = self.upsample4(MFB_3_r)
        M_3 = self.conv_256_64(M_3)
        ...
        M_4 = self.conv_512_64(MFB_4_r)
        M_4 = self.upsample8(M_4)
        # M_4 = self.conv_1024_64(M_4)
        # print("M_4",M_4.shape)

        # print("MFB_5_r",MFB_5_r.shape)
        M_5 = self.conv_512_64(MFB_5_r)
        ...
        # out3 = out1 + S_2
        # print("KGM4", KGM4.shape)
        # print("KGM3", KGM3.shape)
        # print("KGM2", KGM2.shape)
        # print("KGM1", KGM1.shape)

        # out = self.upsample2(KGM4)
        out = self.conv_1x1_output(KGM4)
        # print('MFB_1_r',MFB_1_r.shape)
        out4 = self.conv_1x1_output (MFB_1_r)
        # out4 = self.upsample2(out4)
        # print('MFB_2_r',MFB_2_r.shape)
        out5 = self.conv_128_1_output(MFB_2_r)
        out5 = self.upsample2(out5)
        # print("MFB_3_r",MFB_3_r.shape)
        out6 = self.conv_256_1_output(MFB_3_r)
        # print("out6",out6.shape)
        out6 = self.upsample4(out6)
        out7 = self.conv_512_1_output(MFB_4_r)
        out7 = self.upsample8(out7)
        out8 = self.conv_512_1_output(MFB_5_r)
        out8 = self.upsample16(out8)
